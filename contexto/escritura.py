...
class Escritor:
    def __init__(self, ubicacion_archivo, texto):
        """
        Constructor por defecto de la clase Escritor. Esta clase se encarga d\
        e guardar texto en archivos de distintos tipos como Word, PDF, CSV, T\
        XT, RTF e imágenes.
    ...
    def escribir_txt(self):
        """
        Especifica que el texto deseado se guardará en un archivo con extensi\
        ón '.txt'.
        """
    ...
    def texto_a_archivo(self, tipo="inferir"):
        ...
        :param tipo: (str) {'inferir', 'txt', 'csv', 'pdf', 'doc', 'docx'} Va\
            lor por defecto: 'inferir'. Define el tipo del archivo en el que \
            se desea guardar el texto.
        """
        if tipo == "inferir":
        ...
        if tipo in ["txt", "csv"]:
            ...
            nueva_ruta = "".join(
                self.ubicacion_archivo.split(".")[:-1]
            ) + "_{}.txt".format(tipo)
...
def escribir_texto(ubicacion_archivo, texto, tipo="inferir"):
    """
    Función que guarda texto en un archivo específico. Permite escoger la rut\
    a del archivo, su tipo y el texto a ser guardado dentro de este archivo.
    ...
    if isinstance(texto, str) or isinstance(texto[0], str):
        ...
        print('El argumento "texto" debe ser un texto o una lista de textos.')
