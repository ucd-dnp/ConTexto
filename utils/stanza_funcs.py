...
def stanza_pipeline(leng, procesors='tokenize, pos, lemma', lemma_model_path='', ner_model_path='', pos_model_path=''):
    ...
    # Intentar crear pipeline. Si el modelo no está descargado, se descarga primero
    try:
...
# opciones de tipo: 'lemma', 'pos', 'tokenize'
    ...
    if in_path == '':
        processor = [i for i in nlp_pipe.loaded_processors if tipo.lower() in str(i).lower()][0]
    ...
    # Si no se especificó una ubicación para el modelo resultante, este se borra
    if borrar_modelo:
        os.remove(out_path)
