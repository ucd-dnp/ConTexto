...
Este ejemplo muestra las principales funcionalidades del módulo :py:mod:`Limpieza <limpieza>` de la librería. También muestra ejemplos de uso de las funciones de limpieza contenidas en el módulo auxiliar `limpieza_aux`, que hace parte de `utils`.
...
Adicionalmente, el módulo :ref:`Limpieza auxiliar <funciones_utils_limpieza_utils>` contiene algunas funciones complementarias que permiten identificar y remover elementos adicionales que puedan entorpecer el análisis de un conjunto de textos.
...
Para evitar este problema, la función :py:func:`utils.limpieza_aux.quitar_coincidenias() <utils.tokenizacion.quitar_coincidenias>` (que a su vez utiliza las funciones :py:func:`utils.limpieza_aux.substrings_en_comun() <utils.tokenizacion.substrings_en_comun>` y :py:func:`utils.limpieza_aux.detectar_coincidencias() <utils.tokenizacion.detectar_coincidencias>` permite, para un conjunto de textos, encontrar y remover coincidencias (cadenas de caracteres) que cumplan una o varias de estas condiciones:
...
Para evitar este problema, la función :py:func:`utils.limpieza_aux.quitar_palabras_atipicas() <utils.tokenizacion.quitar_palabras_atipicas>` (que a su vez utiliza las funciones :py:func:`utils.limpieza_aux.caracteres_repetidos() <utils.tokenizacion.caracteres_repetidos>`, :py:func:`utils.limpieza_aux.caracteres_consecutivos() <utils.tokenizacion.caracteres_consecutivos>` y :py:func:`utils.limpieza_aux.consonantes_consecutivas() <utils.tokenizacion.consonantes_consecutivas>`) permite, para un conjunto de textos, encontrar y remover palabras que cumplan una o varias de estas condiciones:
